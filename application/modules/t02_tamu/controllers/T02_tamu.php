...
class T02_tamu extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('T02_tamu_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 't02_tamu?q=' . urlencode($q);
            $config['first_url'] = base_url() . 't02_tamu?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 't02_tamu';
            $config['first_url'] = base_url() . 't02_tamu';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->T02_tamu_model->total_rows($q);
        $t02_tamu = $this->T02_tamu_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            't02_tamu_data' => $t02_tamu,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        // $this->load->view('t02_tamu/t02_tamu_list', $data);
        $data['_view'] = 't02_tamu/t02_tamu_list';
    ...
    {
        $row = $this->T02_tamu_model->get_by_id($id);
        if ($row) {
            $data = array(
				'idtamu' => $row->idtamu,
				'TripNo' => $row->TripNo,
            ...
            $this->load->view('t02_tamu/t02_tamu_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('t02_tamu'));
        }
    }
    ...
    public function create()
    {
        $data = array(
            'button' => 'Simpan',
            'action' => site_url('t02_tamu/create_action'),
        ...
        // $this->load->view('t02_tamu/t02_tamu_form', $data);
        $data['_view'] = 't02_tamu/t02_tamu_form';
        $data['_caption'] = 'Data Tamu';
        $this->load->view('_00_dashboard/_00_dashboard_view', $data);
    }
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->T02_tamu_model->insert($data);
    ...
    public function update($id)
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Simpan',
                'action' => site_url('t02_tamu/update_action'),
    ...
    public function update_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->T02_tamu_model->update($this->input->post('idtamu', TRUE), $data);
    ...
    public function excel()
    {
		...
		foreach ($this->T02_tamu_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            't02_tamu_data' => $this->T02_tamu_model->get_all(),
            'start' => 0
        );
...
/* Location: ./application/controllers/T02_tamu.php */
