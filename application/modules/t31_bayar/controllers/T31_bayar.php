...
class T31_bayar extends CI_Controller
{
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
             ...
             */
            $row = $this->T30_tamu_model->getByTripNo($this->input->post('TripNo',TRUE));
            if ($row) {
                 ...
                 */
                $insert_id = $this->T31_bayar_model->getInsertId();

                /**
                 * simpan data setiap tamu ke tabel detail
                */
                $data = $this->T30_tamu_model->getAllByTripNo($this->input->post('TripNo',TRUE));
                foreach ($data as $row) {
                    $detail = [
                        'idbayar' => $insert_id,
                        'tamu' => $row->idtamu,
                ...
                // redirect(site_url('t31_bayar'));
                redirect(site_url('t31_bayar/proses/' . $insert_id));
            } else {
    ...
    public function proses($id)
    {
        ...
        if ($row) {
            ...
            $data = array(
                'button' => 'Simpan',
                'action' => site_url('t31_bayar/proses_action'),
				'idbayar' => set_value('idbayar', $row->idbayar),
				'TripNo' => set_value('TripNo', $row->TripNo),
				'TripTgl' => set_value('TripTgl', $row->TripTgl),
				'Total' => set_value('Total', $row->Total),
...
/* Generated by Harviacode Codeigniter CRUD Generator 2021-04-08 11:01:07 */
/* http://harviacode.com */
